...
const Button: React.FC<ButtonProps> = ({ label, ricon, licon, color, onClick }) => {
    return (
      <button className={`relative w-full py-5 px-4 rounded-lg font-bold transition-colors hover:bg-opacity-80 active:bg-opacity-80 ${color}`} onClick={onClick}>
        {ricon && <Image src={ricon} alt="アイコン" className="absolute w-6 left-4 top-1/2 transform -translate-y-1/2" />} {/* アイコンを表示 */}
        {label} {/* プロパティからラベルを表示 */}
        {licon && <Image  src={licon} alt="アイコン" className="absolute w-6 right-4 top-1/2 transform -translate-y-1/2" />} {/* アイコンを表示 */}
      </button>
    );
  };
